...
BEGIN
    ...
    EXECUTE format($$     
        ...
        WITH speed_links AS (
            ...
            FROM  		here.ta_path
            INNER JOIN 	congestion.%I links USING (link_dir)
            WHERE 		(dt >= %L AND dt < %L) 
        ...
        */
        tt_hr AS (
            ...
            INNER JOIN 	(SELECT segment_id, sum(length) AS total_length FROM congestion.%I GROUP BY segment_id)a USING (segment_id)
        ...
        */
        ...
        WHERE           retired.segment_id IS NULL OR (%L >= valid_from AND %L < valid_to);

        $$ , network_table, _dt, _dt, network_table, _dt, _dt);
END;
$BODY$;
