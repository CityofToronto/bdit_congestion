...
WITH centreline_daily AS (
    ...
    WHERE
        ...
        AND yr::int = EXTRACT(YEAR FROM _dt - interval '1 year') -- select last year's baseline 
        AND holiday.dt IS NULL -- exclude holidays
    GROUP BY
        uid, dt, hr, length_w_data, baseline_10pct, baseline_25pct, segment_set_length, cent_length
...
SELECT
    ...
    round(PERCENTILE_CONT(0.5) WITHIN GROUP (
        ORDER BY tt
    )::numeric, 2) AS median_tt,
    round(PERCENTILE_CONT(0.85) WITHIN GROUP (
        ORDER BY tt
    )::numeric, 2) AS pct_85_tt,
    round(PERCENTILE_CONT(0.95) WITHIN GROUP (
        ORDER BY tt
    )::numeric, 2) AS pct_95_tt,
