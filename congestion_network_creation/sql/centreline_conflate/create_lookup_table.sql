------------------------------------
-- Contraction
-- First use contraction to gather centreline between all segments
-- using linear contraction, length as cost
-- forbidden vertices are all other intersections (so it does not contract through other ints)
------------------------------------
...
left join congestion.temp_network_segments_24_4 on  a.node_id  = start_vid and b.node_id = end_vid
...
left join gis_core.routing_centreline_directional_higher_rc cent on source = a.intersection_id and target = b.intersection_id
...
from seg_to_route
left join congestion.network_int_px_24_4 s on start_vid = node_id
left join congestion.network_int_px_24_4 e on end_vid = e.node_id),
results AS (
...
agg AS (
    ...
    FROM results d
    JOIN gis_core.routing_centreline_directional_higher_rc r
      ON d.edge = r.id
...
create table natalie.temp_1_to_1_check_highway as
WITH seg_to_route AS (
    SELECT segment_id, start_vid, end_vid
    FROM congestion.temp_network_segments_24_4
...
from natalie.temp_1_to_1_check_highway;
...
drop table natalie.temp_1_to_1_check_highway;
