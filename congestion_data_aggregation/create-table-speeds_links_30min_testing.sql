...
SELECT X.segment_id, X.link_dir, X.datetime_bin, 
...
harmean(pct_50) AS spd_avg_all, COUNT (DISTINCT b.tx) AS count_all
...
(
SELECT a.segment_id, a.link_dir, (datetime_bin(b.tx,30)) AS datetime_bin,
harmean(pct_50) AS spd_avg_hc, COUNT (distinct (datetime_bin(b.tx,30)) )  AS count_hc
FROM congestion.segment_link_test a
INNER JOIN congestion.data_fall2019_5min b
USING (link_dir)
WHERE confidence >= 30
