--returned 12,163 (2 missing segments are 967 and 2704 as there's only one row of data (only 1 timebin) for each segment)
--took 1.5 min

CREATE TABLE congestion.tt_segments_baseline_v2 AS 
...
FROM congestion.tt_segments_30min_v2
...
SELECT daytime.segment_id, segments_v2.length AS seg_length,
overnight.tt_baseline_overnight,
daytime.tt_baseline_10pct, daytime.tt_baseline_15pct, tt_baseline_20pct, tt_baseline_25pct,
segments_v2.length / overnight.tt_baseline_overnight * 3.6 AS spd_baseline_overnight,
segments_v2.length / overnight.tt_baseline_overnight_med * 3.6 AS spd_baseline_overnight_med,
segments_v2.length / daytime.tt_baseline_10pct * 3.6 AS spd_baseline_10pct,
segments_v2.length / daytime.tt_baseline_15pct * 3.6 AS spd_baseline_15pct,
segments_v2.length / daytime.tt_baseline_20pct * 3.6 AS spd_baseline_20pct,
segments_v2.length / daytime.tt_baseline_25pct * 3.6 AS spd_baseline_25pct,
segments_v2.length / daytime.tt_baseline_25pct_med * 3.6 AS spd_baseline_25pct_med
FROM daytime 
JOIN overnight USING (segment_id)
JOIN congestion.segments_v2 USING (segment_id)
